  ...
  <div class="sidebar-content">
    <ul class="sidebar-list">
      ...
      <li>
        <a href="#">房间管理</a>
        <ul class="sub-menu">
          <li><a href="/thinkphp_hotel/index.php/Admin/System/login_out"><i class="icon-font">&#xe995;</i>新增房间</a></li>
      ...
      <div class="result-wrap">
        <div class="result-content">
          <table class="result-tab" width="100%">
            ...
            <?php
 $pagesize=20; $sql = "select a.orderid,a.roomid,a.cardid,a.entertime,a.leavetime,b.typename,a.linkman,a.phone,a.ostatus,a.oremarks,b.price from orders a, roomtype b where a.typeid=b.typeid and a.ostatus='是' and a.oremarks='否' and a.".@$_POST["search-type"]." like ('%".@$_POST["keywords"]."%')"; $rs=mysqli_query($conn,$sql); if(!$rs) { echo "<br />"; echo "<span style=margin-left:200px>无满足条件的记录！</span>"; exit; } $recordcount=mysqli_num_rows($rs); $pagecount=($recordcount-1)/$pagesize+1; $pagecount=(int)$pagecount; $pageno=@$_GET["pageno"]; if($pageno=="") { $pageno=1; } if($pageno<1) { $pageno=1; } if($pageno>$pagecount) { $pageno=$pagecount; } $startno=($pageno-1)*$pagesize; $sql="select a.orderid,a.roomid,a.cardid,a.entertime,a.leavetime,b.typename,a.linkman,a.phone,a.ostatus,a.oremarks,b.price from orders a, roomtype b where a.typeid=b.typeid and a.ostatus='是' and a.oremarks='否' and a.".@$_POST["search-type"]." like ('%".@$_POST["keywords"]."%') order by roomid desc limit $startno,$pagesize"; $rs=mysqli_query($conn,$sql); if(!$rs) { echo "<br />"; echo "<span style=margin-left:200px>无满足条件的记录！</span>"; exit; } while($rows=mysqli_fetch_assoc($rs)) { echo "<tr>"; echo "<th class='tc'>".$rows["orderid"]."</th>"; echo "<th class='tc'>".$rows["roomid"]."</th>"; echo "<th class='tc'>".$rows["cardid"]."</th>"; echo "<th class='tc'>".$rows["entertime"]."</th>"; echo "<th class='tc'>".$rows["leavetime"]."</th>"; echo "<th class='tc'>".$rows["typename"]."</th>"; echo "<th class='tc'>".$rows["linkman"]."</th>"; echo "<th class='tc'>".$rows["phone"]."</th>"; echo "<th class='tc'>".$rows["ostatus"]."</th>"; echo "<th class='tc'>".$rows["oremarks"]."</th>"; echo "<th class='tc'>".$rows["price"]."</th>"; echo "<th class='tc'>"; echo "<a href='update.php?olrid=".$rows["roomid"]."' class='link-update'>办理入住</a>"; echo "</th>"; echo "</tr>"; } ?>
          </table>
          <div class="list-page">
            <tr>
              <?php
